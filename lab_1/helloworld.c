...
int main (void) {
    ...
    #define PHRASE_LENGTH 1

    char word[PHRASE_LENGTH] = "M";
    char blank_word [21]= "                    ";
    char temp [1]= "";
    ...
    while(a){
            ...
            temp[0] = word[0];
            int i = 0;
            for(i = 0; i<(PHRASE_LENGTH-1); i++){
                word[i] = word[i+1];
            }
            word[PHRASE_LENGTH -1]=blank_word[0];
            int j = 0;
            for(j=0; j<19; j++){
                blank_word[j] = blank_word[j+1];
            }
            blank_word[19]=temp[0];
            //int k = 0;
            //for(k = 0; k<20; k++){
            lcd_printf("%s", blank_word);
            //}
